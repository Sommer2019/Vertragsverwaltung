...
class RepoSQLTest {
    ...
    static void initDatabase() throws SQLException {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            ...
            // Tabelle für Fahrzeug (benötigt für den Join)
            stmt.execute("CREATE TABLE fahrzeug ("
                    + "amtlichesKennzeichen VARCHAR(255) PRIMARY KEY,"
                    + "hersteller VARCHAR(255),"
                    + "typ VARCHAR(255),"
                    + "hoechstgeschwindigkeit INT,"
                    + "wagnisskennziffer INT"
                    + ")");

            // Tabelle für Partner (benötigt für den Join)
            // Vereinfachung: Es wird hier nur über den Vornamen gejoint.
            stmt.execute("CREATE TABLE partner ("
                    + "vorname VARCHAR(255) PRIMARY KEY,"
            ...
            // Tabelle für Verträge
            stmt.execute("CREATE TABLE vertrage ("
                    + "vsnr INT PRIMARY KEY,"
                    + "abrechnungszeitraum_monatlich BOOLEAN,"
                    + "preis DOUBLE,"
                    + "versicherungsbeginn DATE,"
                    + "versicherungsablauf DATE,"
                    + "antragsDatum DATE,"
                    + "fahrzeug_kennzeichen VARCHAR(255),"
                    + "fahrzeug_hersteller VARCHAR(255),"
                    + "fahrzeug_typ VARCHAR(255),"
                    + "fahrzeug_hoechstgeschwindigkeit INT,"
                    + "fahrzeug_wagnisskennziffer INT,"
                    + "partner_vorname VARCHAR(255),"
                    + "partner_nachname VARCHAR(255),"
                    + "partner_geschlecht VARCHAR(1),"
                    + "partner_geburtsdatum DATE,"
                    + "partner_land VARCHAR(255),"
                    + "partner_strasse VARCHAR(255),"
                    + "partner_hausnummer VARCHAR(255),"
                    + "partner_plz VARCHAR(255),"
                    + "partner_stadt VARCHAR(255),"
                    + "partner_bundesland VARCHAR(255)"
                    + ")");

            // Tabelle für Faktoren
            stmt.execute("CREATE TABLE faktoren ("
                    + "id INT PRIMARY KEY,"
                    + "factor DOUBLE,"
                    + "factorage DOUBLE,"
                    + "factorspeed DOUBLE"
                    + ")");
            // Dummy-Datensatz für Faktoren einfügen
            stmt.execute("INSERT INTO faktoren (id, factor, factorage, factorspeed) VALUES (1, 1.0, 1.0, 1.0)");
        }
    }
